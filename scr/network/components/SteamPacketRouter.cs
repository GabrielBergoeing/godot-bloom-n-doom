...
public partial class SteamPacketRouter : Node
{
    ...
    public void RegisterHandler(byte packetId, Action<CSteamID, byte[]> handler)
    {
        _handlers[packetId] = handler;
    }

    public void RoutePacket(CSteamID sender, byte[] data)
    {
        if (data.Length == 0)
            return;
        ...
        {
            GD.PrintErr($"[SteamPacketRouter] No handler for packet {packetId}");
            return;
        }
